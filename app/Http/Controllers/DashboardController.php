...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->role == 'Karyawan') {
            ...
            return view('dashboard.index')->with('cek_presensi', $cek_presensi)->with('presensis', $presensis)->with('pengaturan_presensi', $pengaturan_presensi);
        } else {
            $karyawan_hadir = presensi::whereDate('created_at', now()->toDateString())->count();

            return view('dashboard.index')->with('jumlah_pelamar', $jumlah_pelamar)->with('pelamar', $pelamar)->with('jumlah_karyawan_baru', $jumlah_karyawan_baru)->with('jumlah_karyawan_aktif', $jumlah_karyawan_aktif)->with('jumlah_karyawan', $jumlah_karyawan)->with('karyawan_hadir', $karyawan_hadir);
        }
